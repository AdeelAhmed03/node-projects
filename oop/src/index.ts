    ...
    while (true) {
        ...
        switch (action) {
            ...
            case 'Toggle Task Completion':
                ...
                const taskChoices = tasks.map((task: { description: any; id: any; }) => ({ name: task.description, value: task.id }));
            ...
            case 'List Tasks':
                ...
                allTasks.forEach((task: { completed: any; description: any; }) => console.log(`- [${task.completed ? 'x' : ' '}] ${task.description}`));
                break;
            case 'Exit':
